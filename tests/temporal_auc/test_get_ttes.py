...
def index_df(
    ...
    subj_included = draw(st.lists(st.booleans(), min_size=data.n_subjects, max_size=data.n_subjects))

    index_rows = []

    for subj, incl in zip(data.subjects, subj_included, strict=True):
        if not incl:
            continue

        index_time = draw(
            st.datetimes(
                min_value=subj.init_time - timedelta(days=7),
                max_value=subj.max_time + timedelta(days=7),
            )
        )
...
@given(data=MEDSData.strategy())
def test_get_raw_tte_matches_manual(data: MEDSData):
